...
def evaluate_fold(rsf, X_test, y_test, metrics):
    """
    Evalúa las predicciones con los datos de test de un fold

    Parámetros:
    rsf (sksurv.ensemble.RandomSurvivalForest): Modelo RSF entrenado
    X_test (pd.DataFrame): Datos de test
    y_test (np.array): Etiquetas de test
...
def explain_local(rsf, explainer, X_test, y_test):
    """
    ...
    for i in range(len(patients)):
        ...
        plt.tight_layout()
        plt.show()
    plot_survival_function(rsf, X_test_sel, y_test_sel)


def plot_survival_function(rsf, X_test, y_test):
    """
    ...
    for i, s in enumerate(surv[:2]):
        label = f'sMCI#{i + 1}'
        plt.plot(rsf.unique_times_, s, label=label)
    for i, s in enumerate(surv[2:], start=2):
        label = f'pMCI#{i - 1}'
        line, = plt.plot(rsf.unique_times_, s, label=label)
        plt.axvline(x=y_test[i][1], linestyle='--', color=line.get_color())
    plt.ylabel('Survival probability')
    plt.xlabel('Time in months')
    plt.xticks(np.arange(rsf.unique_times_.min(), rsf.unique_times_.max() + 1, 12))
    plt.legend()
...
def main():
    ...
    rsf = RandomSurvivalForest(n_estimators=1000, min_samples_split=10, min_samples_leaf=15, n_jobs=-1, random_state=1234)
    for fold in range(1, args.n_folds + 1):
        X_train, y_train, X_test, y_test = read_data(f'data/imputed/{args.viscode}', fold)
        rsf.fit(X_train, y_train)
        metrics = evaluate_fold(fold, rsf, X_test, y_test, metrics)
        explainer = shap.Explainer(rsf.predict, X_train, feature_names=X_train.columns, seed=1234)
